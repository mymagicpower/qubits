...
class DisplayedToolbox {
    ...
    constructor(
        ...
        /** @type {!int} */
        this.groupHeight = Config.TOOLBOX_GROUP_ROWS;
    ...
    withCustomGatesInserted(customGateSet) {
        ...
        for (let i = 0; i < Math.max(1, customGateSet.gates.length); i += this.groupHeight*Config.TOOLBOX_GROUP_COLS) {
            let group = {
                hint: '',
                gates: [undefined, undefined, undefined, undefined, undefined, undefined]
            };
            for (let j = 0; j < this.groupHeight*Config.TOOLBOX_GROUP_COLS && i + j < customGateSet.gates.length; j++) {
    ...
    gateDrawRect(groupIndex, gateIndex) {
        let dx = gateIndex % Config.TOOLBOX_GROUP_COLS;
        let dy = Math.floor(gateIndex / Config.TOOLBOX_GROUP_COLS);
    ...
    groupLabelRect(groupIndex) {
        if (this.labelsOnTop) {
            ...
            return new Rect(c.x - Config.TOOLBOX_GATE_SPAN, c.y, Config.TOOLBOX_GATE_SPAN * Config.TOOLBOX_GROUP_COLS, 20);
        }

        let r = this.gateDrawRect(groupIndex, this.groupHeight*Config.TOOLBOX_GROUP_COLS - 2);
        let c = new Point(r.x + Config.TOOLBOX_GATE_SPAN - Config.TOOLBOX_GATE_SPACING / 2, r.bottom());
        return new Rect(c.x - Config.TOOLBOX_GATE_SPAN, c.y+2, Config.TOOLBOX_GATE_SPAN * Config.TOOLBOX_GROUP_COLS, 20);
    }
